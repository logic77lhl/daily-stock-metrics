...
def build(today=None):
    ...
    quality = _pct100(df["ROE%"]) * 0.6 + _pct100(df["毛利率%"]) * 0.4
    growth = _pct100(df["净利同比%"]) * 0.7 + _pct100(df["营收同比%"]) * 0.3
    value = (100 - _pct100(df["PE5年分位%"])) * 0.5 + (100 - _pct100(df["PB5年分位%"])) * 0.5
    scale = _pct100(df["市值(亿)"])
    df["质量分"] = quality.round(0).astype(int)
    df["成长分"] = growth.round(0).astype(int)
    df["估值分"] = value.round(0).astype(int)
    df["规模分"] = scale.round(0).astype(int)
    df["综合分"] = (quality * 0.30 + growth * 0.25 + value * 0.35 + scale * 0.10).round(1)
    df = df.sort_values("综合分", ascending=False).head(TOP_N).reset_index(drop=True)
...
def main():
    ...
    md = ("## 💎 价值投资标的 TOP%d\n\n%s\n\n"
          ...
          "仅保留盈利且市值≥300亿；仅供研究参考，不构成投资建议" % (len(df), "\n".join(md_lines)))

    html = f"""<!DOCTYPE html>
<html lang="zh-CN"><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>💎 价值投资标的 TOP{len(df)} - {today}</title>
<style>
...
<h1>💎 价值投资标的 TOP{len(df)} <span style="font-size:13px;color:#888">{today}</span></h1>
<div class="sub">A股+港股通市值前100中筛选 · 质量30% + 成长25% + 估值35% + 规模10%（市况内百分位计分）</div>
{_html_table(df)}
